...
Route::prefix('api')
    ->group(function () {
        Route::get('/multiple/{dummy}/bindings/{dummyChild}', fn (RouteBindingRequest $request, Dummy $dummy, DummyChild $dummyChild) => null)
            ->name('api.multiple.bindings');
        Route::get('/model/{dummy:email}', fn (RouteBindingRequest $request, Dummy $dummy) => null)
            ->name('api.model.column.binding');
        Route::get('/built-in/{int}', fn (RouteBindingRequest $request, int $int) => null)
            ->name('api.built.in.binding');
        Route::get('/class/{class}', fn (RouteBindingRequest $request, DummyService $class) => null)
            ->name('api.class.binding');
        Route::get('/enum/{enum}', fn (RouteBindingRequest $request, DummyEnum $enum) => null)
